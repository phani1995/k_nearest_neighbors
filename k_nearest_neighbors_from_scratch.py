...
for index_x_test,row_x_test in X_test.iterrows():
    ...
    # Slicing so we require only k nearest neighbors
    nearest_neighbors = nearest_neighbors[:k]
    
    # Counting which class have the maximum nearby instances
    counting_dict = {}
    for ele in nearest_neighbors:
        if ele[1] not in list(counting_dict.keys()):
            counting_dict[ele[1][0]] = 1
        else:
            counting_dict[ele[1][0]]+= 1
    
    # Filtering the nearest neighbour
    max_value = 0 
    max_key = None
